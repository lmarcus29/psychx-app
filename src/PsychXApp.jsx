...
async function loadPatients() {
  try {
    const v71 = localStorage.getItem(STORAGE_KEY);
    if (v71) return JSON.parse(v71).map(migratePatient);
...
<div style="margin-top:8px;font-size:8px;color:#94a3b8;text-align:center">Generated by PsychX v0.71 · ${new Date().toLocaleString()} · Review all fields before submission</div>
</div>
</body></html>`;
}
...
const ftr = (n = "") => `<div class="ftr"><div>PsychX v0.71${n ? " · " + n : ""}</div><div>${new Date().toLocaleString()}</div></div>`;
...
export default function App() {
  ...
  const deletePatient = (id) => {
    ...
    setAppDeleteConfirm({
      title: "Delete Patient Record",
      description: "Permanently delete " + (pt?.firstName||'') + " " + (pt?.lastName||'') + "? All sessions, PAs, and notes will be removed permanently.",
      detail: "Patient: " + (pt?.firstName||'') + " " + (pt?.lastName||'') + " - MRN: " + (pt?.psychxMRN||'N/A') + " - Sessions: " + ((pt?.sessions||[]).length),
      onConfirm: () => {
  ...
  return (
    <div style={S.app}>
      {appDeleteConfirm && <ConfirmDeleteModal {...appDeleteConfirm} onCancel={() => setAppDeleteConfirm(null)} />}
      <div style={S.sidebar}>
        <div style={{ padding: "24px 20px 20px" }}>
          ...
          <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 10, marginTop: 8 }}>v0.71 · {patients.length} patient{patients.length !== 1 ? "s" : ""}</div>
        </div>
